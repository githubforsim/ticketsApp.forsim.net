...
document.getElementById("modif").addEventListener("click", function (event) {
  ...
  if (totalFiles.length > 0) {
    ...
    ajaxRequest("POST", `/ticketsApp/config/routes.php/add_attachments_admin/${ticketId}`, openTickets, data);
  }
});

//Renvoi à la page de tickets ouverts - URL CORRIGÉE
function openTickets() {
  window.location.href = "/ticketsApp/admin/tickets/open";
}

//requête AJAX au serveur pour marquer un ticket comme "fermé" - URL CORRIGÉE
...
//requête AJAX au serveur pour marquer un ticket comme "résolu" - URL CORRIGÉE
function setSolve(ticketId) {
  ajaxRequest("POST", `/ticketsApp/config/routes.php/setSolve/${ticketId}`, function (response) {
    var data = JSON.parse(response);
    if (data.success) {
      // URL CORRIGÉE - vers la route propre
      window.location.href = "/ticketsApp/admin/tickets/solved";
    } else {
      console.error(data.error);
    }
  });
...
//Gestion d'evenement on click sur le bouton close
document.addEventListener("DOMContentLoaded", function () {
  ...
  //Si clic sur bouton solve appel de la fonction setSolve
  if (solveButton) {
